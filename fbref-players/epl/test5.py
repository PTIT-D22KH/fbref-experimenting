...
def create_radar_chart(params, low, high, lower_is_better, player1_values, player2_values, player1_name, player2_name, team1_name, team2_name):
    ...
    rings_inner = radar.draw_circles(ax=axs['radar'], facecolor='#f77b83', edgecolor='#fe2837')
    radar_output = radar.draw_radar_compare(player1_values, player2_values, ax=axs['radar'], kwargs_radar={'facecolor': '#f9c728', 'hatch': '.', 'alpha': 1}, kwargs_compare={'facecolor': '#e6dedc', 'edgecolor': '#1a1414', 'hatch': '/', 'alpha': 1})
    radar_poly, radar_poly2, vertices1, vertices2 = radar_output
    radar_output2 = radar.draw_radar_compare(player1_values, player2_values, ax=axs['radar'], kwargs_radar={'facecolor': 'None', 'edgecolor': '#646366'}, kwargs_compare={'facecolor': 'None'})
    range_labels = radar.draw_range_labels(ax=axs['radar'], fontsize=25, fontproperties=serif_extra_light.prop)
    param_labels = radar.draw_param_labels(ax=axs['radar'], fontsize=25, fontproperties=serif_regular.prop)
    ...
    endnote_text = axs['endnote'].text(0.99, 0.5, 'Inspired By: StatsBomb / Rami Moghadam', fontsize=15, fontproperties=serif_extra_light.prop, ha='right', va='center')
    title1_text = axs['title'].text(0.01, 0.65, player1_name, fontsize=25, color='#01c49d', fontproperties=rubik_regular.prop, ha='left', va='center')
    ...
    title3_text = axs['title'].text(0.99, 0.65, player2_name, fontsize=25, fontproperties=rubik_regular.prop, ha='right', va='center', color='#d80499')
